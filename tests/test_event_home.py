...
- **期待する結果**: Badge NEW hiển thị đúng vị trí; không xuất hiện trên dòng không có nhãn
    ...
    def test_04_verify_new_badge_visibility(self, access_to_home_screen: Page):
        ...
        visible_badge_count = sum(
            1 for index in range(row_count) if event_home.has_new_badge(index)
        )
        assert visible_badge_count > 0, (
            f"Expected at least 1 NEW badge in announcement list, but found {visible_badge_count}"
        )
