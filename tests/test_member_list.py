...
@allure.story("会員リスト")
@description_md(
    "Test cases 会員リスト_001 – 会員リスト_008"
)
class Test会員管理_会員リスト:
    ...
    # 会員リスト_003
    # -------------------------------------------------------------
    @allure.title("会員リスト_003: Verify sắp xếp theo 会員ID sau khi click header")
    @description_md(
        """
        - **前提条件**: Đã login (account bất kỳ) và đang ở màn hình 会員リスト
    ...
    # 会員リスト_004
    # -------------------------------------------------------------
    @allure.title("会員リスト_004: Verify nút 検索 mở panel tìm kiếm")
    @description_md(
        """
        - **前提条件**: Đã login (account bất kỳ) và đang ở màn hình 会員リスト
...
- **期待する結果**: 各行の状態が「有効」または「無効」であり、空欄でないこと
    ...
    # 会員リスト_008
    # -------------------------------------------------------------------
    @allure.title("会員リスト_008: Verify chuyển đổi giữa các sub-tab (会員属性の設定, 会員登録フォーム, アプリ利用者, 会員退会設定)")
    @description_md(
        """
- **前提条件**: 会員リスト画面を表示中
- **テスト手順**:
