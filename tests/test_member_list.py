...
- **期待する結果**:
  ...
  - Text 会員リスト（3） hiển thị đúng số lượng 会員 hiện tại
    ...
    def test_member_list_tab_is_active_by_default_and_shows_correct_count(
        self, access_to_member_list_screen: MemberListPage
    ):
        member_list = access_to_member_list_screen
        ...
        expected_heading = MEMBER_LIST_HEADING_WITH_COUNT.format(count=expected_count)
